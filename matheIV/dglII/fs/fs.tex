...
\includegraphics[scale=0.4]{trafo}

\section{Fourier}
\[
f(z) = \sum\limits_{k=0}^\infty a_k cos(k \omega z) + b_k sin(k \omega z) \mbox{ mit } a_k = \frac{4}{T} \int\limits_{0}^{\frac{T}{2}} u_0(x) cos(k \omega x) dx
 \mbox{ unt } b_k = \frac{4}{T} \int\limits_{0}^{\frac{T}{2}} u_0(x) sin(k \omega x) dx
\]

\section{Wellengleichungen}
\[ \mbox{Produktansatz: } u(x,t)=\sum\limits_{k=1}^\infty \left( a_k cos(kt) + b_k sin(kt) \right) sin(kt) \]
